...
export default function HonorairesPage() {
  const { isDirecteurGeneral, isGestionnaire, isAccountant, isCaissiere, canMarkHonorairesPaid } = useFinancialPermissions();
  const canView = isDirecteurGeneral || isGestionnaire || isAccountant || isCaissiere;
  const canPay = canMarkHonorairesPaid || isDirecteurGeneral || isGestionnaire || isAccountant;
  ...
  async function loadData() {
    ...
    try {
      ...
      setHonoraires(
        rows.map((r: any) => ({
          ...r,
          montant_base: Number(r.montant_base ?? r.montant_du),
          montant_du: Number(r.montant_du),
          prorata_ratio: Number(r.prorata_ratio ?? 1),
  ...
  const filtered = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      list = list.filter(
        h =>
          h.medecin_nom.toLowerCase().includes(q) ||
          h.libelle_acte.toLowerCase().includes(q) ||
          (h.reference_etat || '').toLowerCase().includes(q) ||
    ...
    for (const h of honoraires) {
      if (!map[h.medecin_id]) {
        const totalVerse = versements
          .filter(v => v.medecin_id === h.medecin_id)
          .reduce((s, v) => s + Number(v.montant), 0);
        map[h.medecin_id] = {
          medecin_id: h.medecin_id,
          medecin_nom: h.medecin_nom,
          total_du: 0,
  ...
  const globalStats = useMemo(
    () => ({
      totalDu: honoraires.reduce((s, h) => s + h.montant_du, 0),
      resteAVerser: Math.max(
        honoraires.reduce((s, h) => s + h.montant_du, 0) -
          versements.reduce((s, v) => s + Number(v.montant), 0),
        0
      ),
  ...
  const hasPartialProrata = useMemo(
    () => honoraires.some(h => h.prorata_ratio < 1),
    [honoraires]
  );

  const prorataCount = useMemo(
    () => honoraires.filter(h => h.prorata_ratio < 1).length,
    [honoraires]
  );
  ...
  function openPay(medecinId: string) {
    ...
    setPayForm({
      montant: summary ? Math.max(summary.solde, 0).toFixed(2) : '',
      devise: 'USD',
      reference: '',
      notes: '',
    });
  ...
  async function handlePay(e: React.FormEvent) {
    ...
    try {
      ...
      const ref =
        payForm.reference ||
        `HON-VER-${new Date().toISOString().slice(0, 10)}-${Date.now().toString(36).slice(-4).toUpperCase()}`;
      ...
      // Mark individual honoraires as paid — oldest first up to amount
      ...
      for (const h of unpaid) {
        ...
        if (remaining >= h.montant_du) {
          await supabase
            .from('honoraires_medecins')
            .update({ statut_paiement: 'paye', paye_le: new Date().toISOString() })
            .eq('id', h.id);
          remaining -= h.montant_du;
        } else {
          break;
        }
      }

      showToast(`Versement de ${fmt(montant)} ${payForm.devise} enregistré`, 'success');
  ...
  function exportCSV() {
    ...
    const lines = [
      'RELEVÉ DES HONORAIRES — MÉDECINS VISITEURS',
      `Période;${new Date(dateFrom).toLocaleDateString('fr-FR')} au ${new Date(dateTo).toLocaleDateString('fr-FR')}`,
      `Généré le;${new Date().toLocaleString('fr-FR')}`,
      `Total honoraires dus;${fmt(globalStats.totalDu)} USD`,
      `Total versé;${fmt(globalStats.totalVerse)} USD`,
      `Reste à verser;${fmt(globalStats.resteAVerser)} USD`,
      `Dont prorata (paiements partiels);${prorataCount} acte(s)`,
      '',
      'Date;Médecin;Acte;Montant Acte;Mode;Taux/Forfait;Montant Base;Prorata;Honoraire Dû;Réf;Statut',
    ];
    for (const h of filtered) {
      lines.push(
        [
          ...
          fmt(h.montant_base),
          `${(h.prorata_ratio * 100).toFixed(0)}%`,
          fmt(h.montant_du),
          h.reference_etat || '-',
          h.statut_paiement === 'paye' ? 'Payé' : 'À payer',
        ].join(';')
      );
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-medium">Accès restreint</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Suivi et versement des honoraires aux médecins visiteurs (prorata inclus)
          </p>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-800">
              Prorata sur paiements partiels détecté
            </p>
            <p className="text-xs text-amber-600 mt-0.5">
              {prorataCount} honoraire{prorataCount > 1 ? 's' : ''} calculé{prorataCount > 1 ? 's' : ''} au prorata
              car la facture n'a été que partiellement payée. Le montant dû reflète la proportion effectivement encaissée.
            </p>
          </div>
        </div>
      )}

      {/* Summary table by medecin */}
      {summaries.length > 0 && (
              ...
              <tbody className="divide-y divide-gray-100">
                {summaries.map(s => (
                  <tr key={s.medecin_id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-right text-green-700 tabular-nums">{fmt(s.total_verse)}</td>
                    <td
                      ...
                      <td className="px-4 py-3 text-center">
                        {s.solde > 0 && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 text-xs font-semibold transition-colors"
                          >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex flex-wrap items-center gap-3">
          ...
          {(['all', 'a_payer', 'paye'] as const).map(s => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                statutFilter === s ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'
              }`}
            >
              {s === 'all' ? 'Tous' : s === 'a_payer' ? 'À payer' : 'Payés'}
            </button>
          ))}
        </div>
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input
            type="text"
            placeholder="Rechercher médecin, acte, référence..."
            value={searchTerm}
            onChange={e => setSearchTerm(e.target.value)}
            className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400"
          />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-sm font-bold text-gray-800 uppercase tracking-wider">
            Détail des honoraires ({filtered.length})
          </h2>
        </div>
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="font-medium text-gray-500">Aucun honoraire sur cette période</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">Honoraire dû</th>
                ...
                {filtered.map(h => (
                  <tr key={h.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap">
                      {new Date(h.date_prestation).toLocaleDateString('fr-FR')}
                    </td>
                    <td className="px-4 py-3 font-medium text-gray-900">{h.medecin_nom}</td>
                    <td className="px-4 py-3 text-gray-600 max-w-[180px] truncate" title={h.libelle_acte}>
                      {h.libelle_acte}
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${
                          h.mode_remuneration === 'pourcentage'
            ...
            <form onSubmit={handlePay} className="p-6 space-y-4">
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-sm font-semibold text-gray-700">
                  {prestataires.find(p => p.id === payingMedecinId)?.nom_complet}
                </p>
                <p className="text-xs text-gray-500 mt-0.5">
                  Solde restant :{' '}
                  <span className="font-bold text-red-600">
                    {fmt(summaries.find(s => s.medecin_id === payingMedecinId)?.solde ?? 0)} USD
                  </span>
                </p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400"
                    required
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setPayForm(f => ({ ...f, devise: e.target.value as 'USD' | 'CDF' }))}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400 resize-none"
                />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-6 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-medium text-sm disabled:opacity-50 flex items-center gap-2"
                >
                  {submitting ? (
      ...
      {showDetailModal && (
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900">Détail de l'honoraire</h3>
              <button onClick={() => setShowDetailModal(null)} className="p-1 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5 text-gray-400" />
              </button>
            </div>
            <div className="p-6 space-y-3">
              <DetailRow label="Date" value={new Date(showDetailModal.date_prestation).toLocaleDateString('fr-FR')} />
              <DetailRow label="Médecin" value={showDetailModal.medecin_nom} />
              <DetailRow label="Acte" value={showDetailModal.libelle_acte} />
              <DetailRow label="N° Facture" value={showDetailModal.invoice_number || '-'} />
              <DetailRow label="Montant acte" value={`${fmt(showDetailModal.montant_acte)} USD`} />
              <DetailRow
                label="Mode"
                value={
                  showDetailModal.mode_remuneration === 'pourcentage'
                    ? `Pourcentage (${showDetailModal.pourcentage}%)`
                    : `Forfait (${showDetailModal.montant_forfait} USD)`
                }
              />
              <DetailRow label="Montant base (avant prorata)" value={`${fmt(showDetailModal.montant_base)} USD`} />
              {showDetailModal.prorata_ratio < 1 ? (
                <div className="flex justify-between items-center gap-4">
                  <span className="text-sm text-gray-500">Prorata</span>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800">
                    <Percent className="w-3 h-3" />
                    {(showDetailModal.prorata_ratio * 100).toFixed(1)}% (paiement partiel)
                  </span>
                </div>
              ) : (
                <DetailRow label="Prorata" value="100% (paiement intégral)" />
              )}
              <DetailRow label="Honoraire dû" value={`${fmt(showDetailModal.montant_du)} USD`} highlight />
              <DetailRow label="Référence" value={showDetailModal.reference_etat || '-'} />
              <DetailRow
                label="Statut"
                value={showDetailModal.statut_paiement === 'paye' ? 'Payé' : 'À payer'}
              />
              {showDetailModal.paye_le && (
                <DetailRow
                  label="Payé le"
                  value={new Date(showDetailModal.paye_le).toLocaleDateString('fr-FR')}
                />
              )}
...
function DetailRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-start gap-4">
      ...
      <span className={`text-sm text-right ${highlight ? 'font-bold text-amber-700' : 'text-gray-900'}`}>
        {value}
      </span>
    </div>
  );
