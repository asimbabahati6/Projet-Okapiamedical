...
interface Prestataire { id: string; nom_complet: string; }
...
export default function CommissionsPage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [comRes, presRes, verRes] = await Promise.all([
        supabase.from('commissions_medecins').select('*')
        ...
        supabase.from('versements_honoraires').select('medecin_id, montant').eq('type_versement', 'commission'),
      ]);
  ...
  const filtered = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      list = list.filter(c => c.medecin_nom.toLowerCase().includes(q) || c.libelle_acte.toLowerCase().includes(q) || (c.reference_etat || '').toLowerCase().includes(q));
    }
    return list;
  ...
  const summaries = useMemo<BenefSummary[]>(() => {
    ...
    for (const c of commissions) {
      if (!map[c.medecin_id]) {
        const tv = versements.filter(v => v.medecin_id === c.medecin_id).reduce((s, v) => s + Number(v.montant), 0);
        map[c.medecin_id] = { medecin_id: c.medecin_id, medecin_nom: c.medecin_nom, total_du: 0, total_verse: tv, solde: 0, count: 0 };
      }
  ...
  const globalStats = useMemo(() => ({
    totalDu: commissions.reduce((s, c) => s + Number(c.montant_du), 0),
  ...
  function openPay(medecinId: string) {
    ...
    setPayForm({ montant: s ? Math.max(s.solde, 0).toFixed(2) : '', devise: 'USD', reference: '', notes: '' });
  ...
  async function handlePay(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const med = prestataires.find(p => p.id === payingId);
      const ref = payForm.reference || `COM-VER-${new Date().toISOString().slice(0, 10)}-${Date.now().toString(36).slice(-4).toUpperCase()}`;
      ...
      for (const c of unpaid) {
        ...
        if (remaining >= Number(c.montant_du)) {
          await supabase.from('commissions_medecins').update({ statut_paiement: 'paye', paye_le: new Date().toISOString() }).eq('id', c.id);
          remaining -= Number(c.montant_du);
        } else break;
      }
  ...
  function exportCSV() {
    ...
    const lines = [
      'RELEVE DES COMMISSIONS',
      `Periode;${new Date(dateFrom).toLocaleDateString('fr-FR')} au ${new Date(dateTo).toLocaleDateString('fr-FR')}`,
      `Genere le;${new Date().toLocaleString('fr-FR')}`,
      '',
      'Date;Beneficiaire;Acte;Montant Acte;Mode;Taux/Forfait;Commission Due;Ref;Statut',
    ];
    for (const c of filtered) {
      lines.push([
        ...
        `"${c.medecin_nom}"`,
        `"${c.libelle_acte}"`,
        Number(c.montant_acte).toLocaleString('fr-FR', { minimumFractionDigits: 2 }),
        c.mode_remuneration === 'pourcentage' ? 'Pourcentage' : 'Forfait',
        c.mode_remuneration === 'pourcentage' ? `${c.pourcentage}%` : `${c.montant_forfait} USD`,
        Number(c.montant_du).toLocaleString('fr-FR', { minimumFractionDigits: 2 }),
        c.reference_etat || '-',
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Suivi et versement des commissions aux apporteurs et intermediaires</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <CKPI label="Total commissions dues" value={globalStats.totalDu} suffix="USD" icon={<DollarSign className="w-5 h-5 text-teal-600" />} iconBg="bg-teal-100" />
        <CKPI label="Reste a verser" value={Math.max(globalStats.totalDu - globalStats.totalVerse, 0)} suffix="USD" icon={<Banknote className="w-5 h-5 text-red-600" />} iconBg="bg-red-100" alert />
        <CKPI label="Total verse" value={globalStats.totalVerse} suffix="USD" icon={<CheckCircle className="w-5 h-5 text-green-600" />} iconBg="bg-green-100" />
        <CKPI label="Nombre d'actes" value={globalStats.count} icon={<Award className="w-5 h-5 text-blue-600" />} iconBg="bg-blue-100" isCount />
      </div>
      ...
      {summaries.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-sm font-bold text-gray-800 uppercase tracking-wider">Solde par beneficiaire</h2>
          </div>
                  ...
                  <tr key={s.medecin_id} className="hover:bg-gray-50/50">
                    ...
                    {canPay && (
                      <td className="px-4 py-3 text-center">
                        {s.solde > 0 && (
                          <button onClick={() => openPay(s.medecin_id)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-teal-50 text-teal-700 hover:bg-teal-100 text-xs font-semibold transition-colors">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <select value={selectedBenef} onChange={e => setSelectedBenef(e.target.value)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm">
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" placeholder="Rechercher..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-bold text-gray-800 uppercase tracking-wider">Detail des commissions ({filtered.length})</h2>
        </div>
        {loading ? (
          <div className="flex items-center justify-center py-20"><div className="animate-spin rounded-full h-8 w-8 border-2 border-teal-600 border-t-transparent" /></div>
        ) : filtered.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{new Date(c.date_prestation).toLocaleDateString('fr-FR')}</td>
                    <td className="px-4 py-3 font-medium text-gray-900">{c.medecin_nom}</td>
                    <td className="px-4 py-3 text-gray-600 max-w-[180px] truncate" title={c.libelle_acte}>{c.libelle_acte}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-gray-700">{Number(c.montant_acte).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${c.mode_remuneration === 'pourcentage' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
                        {c.mode_remuneration === 'pourcentage' ? `${c.pourcentage}%` : `${c.montant_forfait} USD`}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right font-bold tabular-nums text-teal-700">{Number(c.montant_du).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</td>
                    <td className="px-4 py-3 font-mono text-xs text-gray-500">{c.reference_etat || '-'}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${c.statut_paiement === 'paye' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>
                        {c.statut_paiement === 'paye' ? 'Paye' : 'A payer'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => setShowDetail(c)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors"><Eye className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-teal-100 flex items-center justify-center"><CreditCard className="w-4 h-4 text-teal-600" /></div>
            ...
            <form onSubmit={handlePay} className="p-6 space-y-4">
              <div className="bg-gray-50 rounded-xl p-3">
                <p className="text-sm font-semibold text-gray-700">{prestataires.find(p => p.id === payingId)?.nom_complet}</p>
                <p className="text-xs text-gray-500 mt-0.5">Solde restant: <span className="font-bold text-red-600">{summaries.find(s => s.medecin_id === payingId)?.solde.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} USD</span></p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Montant *</label>
                  <input type="number" step="0.01" min="0.01" value={payForm.montant} onChange={e => setPayForm(f => ({ ...f, montant: e.target.value }))} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Devise</label>
                  <select value={payForm.devise} onChange={e => setPayForm(f => ({ ...f, devise: e.target.value as 'USD' | 'CDF' }))} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400">
              ...
              <div>
                ...
                <input type="text" value={payForm.reference} onChange={e => setPayForm(f => ({ ...f, reference: e.target.value }))} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400" placeholder="N de cheque, virement..." />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                <textarea rows={2} value={payForm.notes} onChange={e => setPayForm(f => ({ ...f, notes: e.target.value }))} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400 resize-none" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowDetail(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-3">
              <CDR label="Date" value={new Date(showDetail.date_prestation).toLocaleDateString('fr-FR')} />
              <CDR label="Beneficiaire" value={showDetail.medecin_nom} />
              <CDR label="Acte" value={showDetail.libelle_acte} />
              <CDR label="N Facture" value={showDetail.invoice_number || '-'} />
              <CDR label="Montant acte" value={`${Number(showDetail.montant_acte).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} USD`} />
              <CDR label="Mode" value={showDetail.mode_remuneration === 'pourcentage' ? `Pourcentage (${showDetail.pourcentage}%)` : `Forfait (${showDetail.montant_forfait} USD)`} />
              <CDR label="Commission due" value={`${Number(showDetail.montant_du).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} USD`} highlight />
              <CDR label="Reference" value={showDetail.reference_etat || '-'} />
              <CDR label="Statut" value={showDetail.statut_paiement === 'paye' ? 'Paye' : 'A payer'} />
              {showDetail.paye_le && <CDR label="Paye le" value={new Date(showDetail.paye_le).toLocaleDateString('fr-FR')} />}
            </div>
          </div>
        </div>
      )}
...
function CKPI({ label, value, suffix, icon, iconBg, alert, isCount }: { label: string; value: number; suffix?: string; icon: React.ReactNode; iconBg: string; alert?: boolean; isCount?: boolean }) {
  return (
...
function CDR({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-start gap-4">
      <span className="text-sm text-gray-500">{label}</span>
      <span className={`text-sm text-right ${highlight ? 'font-bold text-teal-700' : 'text-gray-900'}`}>{value}</span>
    </div>
  );
}
